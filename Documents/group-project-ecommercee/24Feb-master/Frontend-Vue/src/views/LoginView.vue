...
export default {
  ...
  login() {
    if (this.username === 'Customer' && this.password === 'customer123') {
      localStorage.setItem('user', JSON.stringify({ username: this.username })); // Save user data
      globalState.isLoggedIn = true; // Update global login state
      this.$router.push({ name: 'booking' }); // Redirect to booking page
    } else {
      this.errorMessage = 'Invalid username or password';
    }
  },
    signUp() {
      localStorage.setItem('user', JSON.stringify({ username: this.newUsername })); // Save user data
